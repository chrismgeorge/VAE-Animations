...
from layers_face import Dense
from utils_face import composeAll, print_
...
class VAE():
    ...
    def __init__(self, architecture=[], d_hyperparams={}, meta_graph=None,
        ...
        if not meta_graph: # new model
            ...
            #print(os.path.basename(meta_graph).split("_vae_"))
            model_datetime, model_name = os.path.basename(meta_graph).split("_vae_")
            #print(model_datetime, model_name)
    ...
    def train(self, X, max_iter=np.inf, max_epochs=np.inf, cross_validate=True,
        ...
        try:
            ...
            min_cost = float('inf')
            while True:
                x = X.next_batch(self.batch_size)
                ...
                if (i >= max_iter): # or ((err_train / i) > 4*min_cost): # or X.epochs_completed >= max_epochs:
                ...
                min_cost = min(err_train / i, min_cost)
